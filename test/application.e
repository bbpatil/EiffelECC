...
feature {NONE} -- Initialization
	...
	make
		...
		do
			...
			ec_key := {OPEN_SSL_ECC}.c_ec_key_new
			...
			signature := {OPEN_SSL_ECC}.c_ecdsa_do_sign (c_string.item, hash.count, ec_key)
			check not_null_signature: signature /= default_pointer end

				-- Third step: verify the created ECDSA signature using ECDSA_do_verify():
			status := {OPEN_SSL_ECC}.c_ecdsa_do_verify (c_string.item, hash.count, signature, ec_key)
				-- Finallt Return value  1 = OK, 0 incorrect signature, other error.
			print ("Status:" + status.out)

			{OPEN_SSL_ECC}.c_ec_group_free (ec_group)
			{OPEN_SSL_ECC}.c_ec_key_free (ec_key)
		end
